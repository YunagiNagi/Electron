...
let startButton = document.getElementById('start_video_button');
startButton.addEventListener('click', function(e) {
  startMedia();
});

let stopButton = document.getElementById('stop_video_button');
...
let mediaButton = document.getElementById('media_button');
...
function addMedia(source) {
  ...
  childElement.id = sourceId;
  childElement.classList.add('media_source');
  ...
  nameElement.innerHTML = sourceName;
  childElement.appendChild(nameElement);

  const brElement = document.createElement("br");
  childElement.appendChild(brElement);

  const imgElement = document.createElement("img");
  imgElement.src = sourceImage.toDataURL({scaleFactor: 0.1});
  childElement.appendChild(imgElement);

  mediaContainer.appendChild(childElement);
}
